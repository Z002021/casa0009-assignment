...
// ----- MAP ----- //
d3.json("https://raw.githubusercontent.com/ngliangwei15/CASA0009-GroupAssignment/main/hotspots_data.json", function(data){
	...
	for (var i = 0; i < data.features.length; i++){
		...
		data.features[i].properties.hotspot = parseInt(data.features[i].properties.hotspot)
		data.features[i].properties.k_cluster = parseInt(data.features[i].properties.k_cluster)
		data.features[i].properties.occ = Number(data.features[i].properties.occ)
		data.features[i].properties.sp_evenness = Number(data.features[i].properties.sp_evenness)
		data.features[i].properties.sp_richness = Number(data.features[i].properties.sp_richness)
		...
		sp_richness = sp_richness.concat(data.features[i].properties.sp_richness)
		sp_evenness = sp_evenness.concat(data.features[i].properties.sp_evenness)
		// set colors
		colors.push(colors_pal[data.features[i].properties.k_cluster-1])
		// get array of metadata
		meta_data.push([hotspots[i], occ_den[i], sp_richness[i], sp_evenness[i]])
	...
	var data_map = [
		{
			...
			z: clusters,
			colorscale: [[0,colors_pal[0]],[1,colors_pal[1]]],
			marker: {
				opacity: 0.6,
				line: {
			...
			showscale: false,
			hovertemplate: "Hotspot: %{location} <br>Occurrence Density: %{properties.occ}/km^2 <br>Species Richness: %{properties.sp_richness}/km^2 <br>Species Evenness: %{properties.sp_evenness}<extra></extra>",
		}
	]
	...
	data_graph.push({
		...
		showlegend: false,
		meta: meta_data,
		hovertemplate: "Hotspot: %{meta[0]} <br>Occurrence Density: %{meta[1]}/km^2 <br>Species Richness: %{x}/km^2 <br>Species Evenness: %{y}<extra></extra>",
	})
	...
	var buttons_hotspot = [{
		method: 'update',
		args: [{'marker.line.color': marker_line_col, 'marker.line.width': marker_line_width}, {'mapbox.center': {lon: map_ctr_lon, lat: map_ctr_lat}, 'mapbox.zoom': map_zoom}, trace_idx],
	...
	for (var k=0; k<n_hotspots; k++) {
		...
		marker_line_col_update = Array(k).fill(marker_line_col).concat(['red']).concat(Array(n_hotspots-(k+1)).fill(marker_line_col))
		marker_line_width_update = Array(k).fill(marker_line_width).concat([marker_line_width*2]).concat(Array(n_hotspots-(k+1)).fill(marker_line_width))
		...
		var buttons_temp = {
			method: 'update',
			args: [{'marker.line.color': [marker_line_col_update], 'marker.line.width': [marker_line_width_update]}, {'mapbox.center': {lon: map_ctr_lon_update, lat: map_ctr_lat_update}, 'mapbox.zoom': map_zoom_update}, trace_idx],
