...
d3.json("https://raw.githubusercontent.com/ngliangwei15/CASA0009-GroupAssignment/main/england_la_sim.json", function(data) {
	...
	d3.csv("https://raw.githubusercontent.com/ngliangwei15/CASA0009-GroupAssignment/main/LA_ind.csv", function(rows) {
	// $.getJSON("http://dev.spatialdatacapture.org:8708/lad", function(rows) {
		...
		let labels = ['Occ. Density (/km^2)', 'Sp. Richness Density (/km^2)', 'Sp. Evenness']
		let ind_labels = {};
		...
		function filter_rows(rows, locations, year) {
			...
			for (i in locations) {
				var rowFiltered = rows.filter(function(row) {
					return (row.LAD21NM === locations[i] && row.year === year)
				})
		...
		var data_map_base = indicators.map(function(indicator) {
			...
			return {
				...
				visible: visibility,
				hovertemplate: 'LAD: %{location}<br>Metric Value: %{z}<extra></extra>',
			}
		})
		...
		for (var k=0; k<indicators.length; k++) {
			window['data_box_'+indicators[k]] = years.map(function(years) {
				var rowsFiltered_box_base = filter_rows(rows, lad, years)
